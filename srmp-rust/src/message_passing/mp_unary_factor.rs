...
use crate::{
    factor_types::{factor_trait::Factor, unary_factor::UnaryFactor},
...
impl UnaryMessageData {
    ...
    pub fn new(cfn: &GeneralCFN, alpha: &FactorOrigin, beta: &FactorOrigin) -> Self {
        ...
        let first_block_indices = UnaryMessageData::_compute_index_adjustment(
            cfn,
            alpha_variables,
            beta_variables,
            beta_ft_len,
        ...
        let second_block_indices = UnaryMessageData::_compute_index_adjustment(
            cfn,
            alpha_variables,
            &difference,
            difference_ft_len,
...
impl MessagePassing for UnaryFactor {
    type MessageData = UnaryMessageData;
    ...
    fn new_message_data(
        ...
        UnaryMessageData::new(cfn, alpha, beta)
    }
    ...
    fn subtract_outgoing_message(&mut self, message: &Self, message_data: &Self::MessageData) {
        let message_data: Option<&UnaryMessageData> = message_data.into();
